...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        ...
        dataloader_config = DataLoaderConfiguration(
            dispatch_batches=False,  # 每个进程独立加载数据
            split_batches=False       # 将批次拆分到多个进程
        )
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_frame_selector_lr is not None:
                lr_mapper["frame_selector"] = self.args.mm_frame_selector_lr
            if self.args.mm_temporal_encoder_lr is not None:
                lr_mapper["temporal_encoder"] = self.args.mm_temporal_encoder_lr
            if len(lr_mapper) > 0:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if not getattr(self.args, "save_all_parameters", False) and (getattr(self.args, "tune_mm_mlp_adapter", False) or (
