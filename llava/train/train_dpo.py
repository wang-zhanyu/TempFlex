...
def preprocess_phi3(sources: List[List[Dict[str, str]]], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    conv: conversation_lib.Conversation = conversation_lib.default_conversation.copy()
    roles: Dict[str, str] = {"human": conv.roles[0], "gpt": conv.roles[1]}
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if has_image:
                round_len = len(tokenizer_image_token(rou.strip(), tokenizer))  # Ignore <bos>
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer))  # Ignore <bos>
            else:
                round_len = len(tokenizer(rou.strip()).input_ids)  # Ignore <bos>
                instruction_len = len(tokenizer(parts[0]).input_ids)  # Ignore <bos>

            round_len += 1  # sep: <|im_end|> takes 1 tokens
            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
            cur_len += round_len
...
def preprocess_thoth(
    ...
    targets:torch.Tensor = input_ids.clone()
    assert conv.sep_style == conversation_lib.SeparatorStyle.THOTH
    ...
    for conversation, target in zip(conversations, targets):
        ...
        # print("[preprocess_thoth] re_rounds len: {}, :{}".format(len(re_rounds), re_rounds))
        ...
        for i, rou in enumerate(re_rounds):
            ...
            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
            a = input_ids[0][cur_len : cur_len + instruction_len]
            # print(i,a)
            # print(i,a[a>=0])
            # print(i,tokenizer.batch_decode([a[a>=0]]))

            cur_len += round_len
        target[cur_len:] = IGNORE_INDEX
        # print("[preprocess_thoth] target: {}".format(tokenizer.batch_decode([target[target>=0]])))
        # print(input_ids)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if "zero3" in training_args.deepspeed:
                rank0_print("#### Initialize reference model #####")
                ref_model = LlavaGemma2ForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            if "zero3" in training_args.deepspeed:
                rank0_print("#### Initialize reference model #####")
                ref_model = LlavaGemmaForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            if "zero3" in training_args.deepspeed:
                rank0_print("#### Initialize reference model #####")
                ref_model = LlavaPhi3ForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            if "zero3" in training_args.deepspeed:
                rank0_print("#### Initialize reference model #####")
                ref_model = LlavaThothForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    trust_remote_code=True,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
...
def train(attn_implementation=None):
    ...
    if "mpt" in model_args.model_name_or_path:
        ...
        tokenizer = transformers.AutoTokenizer.from_pretrained(model_args.model_name_or_path, cache_dir=training_args.cache_dir, model_max_length=training_args.model_max_length, use_fast=True, trust_remote_code=True, padding_side="right")
