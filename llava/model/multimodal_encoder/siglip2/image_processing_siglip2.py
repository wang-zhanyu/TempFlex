...
from llava.model.qwen2_5_vl.image_utils import make_flat_list_of_images
...
def get_image_size_for_max_num_patches(
    ...
    max_pixels = patch_size * patch_size * max_num_patches * 4
    min_pixels = patch_size * patch_size * 4 * 4

    if h_bar * w_bar > max_pixels:
        beta = math.sqrt((image_height * image_width) / max_pixels)
        h_bar = math.floor(image_height / beta / factor) * factor
        w_bar = math.floor(image_width / beta / factor) * factor
    elif h_bar * w_bar < min_pixels:
        beta = math.sqrt(min_pixels / (image_height * image_width))
        h_bar = math.ceil(image_height * beta / factor) * factor
        w_bar = math.ceil(image_width * beta / factor) * factor
    return h_bar, w_bar
...
class Siglip2ImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        for image in images:
            ...
            patches, mask = pad_along_first_dim(patches, patches.shape[0]) # remove mask
            num_patches_height = image.shape[0] // patch_size
            num_patches_width = image.shape[1] // patch_size

            # 1 is the temporal dimension, which is always 1 for image
            spatial_shapes.append((1, num_patches_height, num_patches_width))
