...
from llava.model.multimodal_encoder.siglip2.image_processing_siglip2 import Siglip2ImageProcessor
...
class Siglip2VisionTransformer(nn.Module):
    def __init__(self, config: Siglip2VisionConfig):
        ...
        self.use_head = False # True if not hasattr(config, "vision_use_head") else config.vision_use_head
        if self.use_head:
    ...
    def forward(
        ...
        # remove self.head by setting self.head to nn.Identity()
...
class Siglip2VisionModel(Siglip2PreTrainedModel):
    ...
    def forward(
        ...
        if spatial_shapes.shape[1] == 3:
            spatial_shapes = spatial_shapes[:, -2:] # only get width and height
...
class SigLip2VisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        self.image_processor = Siglip2ImageProcessor.from_pretrained(vision_tower)
    ...
    def load_model(self, device_map=None):
        ...
        self.vision_tower = Siglip2VisionModel.from_pretrained(self.vision_tower_name, device_map=device_map, attn_implementation='flash_attention_2')
    ...
    def forward(self, images, grid_thw):
        if type(images) is list:
            ...
            image_forward_outs = self.vision_tower(images[0].to(device=self.device, dtype=self.dtype), None, grid_thw[0]) #.to(images.dtype)
            image_features = image_forward_outs[0].to(self.dtype)
            # global_features = image_forward_outs[1].to(self.dtype)
